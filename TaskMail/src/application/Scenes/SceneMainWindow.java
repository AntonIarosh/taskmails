...
public class SceneMainWindow implements mainWindowUser {
	...
	public Scene createNewScene() {
	    ...
	    // Аккордион меню ----/
			...
			Button readTask = new Button("Чтение задачи из файла");
		...
		push.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent e) {
				LinkedList<Integer> all = getAllIds(); // спиок айди до отправки
	...
	// --------------------------------- Конец прикрепления файлов -----------------------------------------/
		...
		pushTaskOun.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent e) {
				...
				// валидация времени - часа начала работы.
				...
				// Валидация времени минут начала работы 
		...
		*/
		...
		for(int i =0; i <24; i++) {
			...
			if (TaskEndforOneHour.getChildren().size() > 0) {
				day.getChildren().addAll(new Label("Задачи, которые нужно\n выполнить к этому часу."),alland);
			}
